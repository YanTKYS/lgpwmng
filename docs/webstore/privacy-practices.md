...
| Location | しない | 扱わない |
| Web history | しない | 閲覧履歴を取得しない。利用者が登録したログイン画面の URL 条件のみを保存する |
...
> 3 項目にチェックを入れますが、いずれも **利用者の端末内に保存するだけ**で、開発者や第三者へ
> 送信することはありません。Dashboard の申告は「拡張が扱うデータの種類」を示すもので、
> 「外部へ送信する」ことを意味するものではありません。この点は Privacy Policy 本文
...
| 観点 | 回答 |
| --- | --- |
| どの種類のユーザーデータを扱うか | 認証情報（ログインID・パスワード・第二パスワード・PIN 等）、組織識別情報（自治体コード・所属コード・職員番号等）、利用者が定義した任意のフォーム入力値、対象ログイン画面の URL（オリジン + パス）、入力欄の構造情報、サービス名・アカウント名・メモ |
